...
export const api = {
  ...
  mangaLinkMatch: (sourceName: string, seriesId: string) => request<MangaLinkMatch>(`/api/manga/link/match?source=${encodeURIComponent(sourceName)}&series_id=${encodeURIComponent(seriesId)}`, { method: "POST" }),
  mangaLink: (sourceName: string, seriesId: string) => request<MangaLink | null>(`/api/manga/link?source=${encodeURIComponent(sourceName)}&series_id=${encodeURIComponent(seriesId)}`),
  setMangaLink: (sourceName: string, seriesId: string, mediaId: number, chapterOffset: number) => request<MangaLink>(`/api/manga/link?source=${encodeURIComponent(sourceName)}&series_id=${encodeURIComponent(seriesId)}`, { method: "PUT", body: JSON.stringify({ media_id: mediaId, chapter_offset: chapterOffset }) }),
  deleteMangaLink: (sourceName: string, seriesId: string) => request<void>(`/api/manga/link?source=${encodeURIComponent(sourceName)}&series_id=${encodeURIComponent(seriesId)}`, { method: "DELETE" }),
  ...
  createReadingEvent: (sourceName: string, seriesId: string, chapterId: string, chapter: number | null) => request<ReadingEventResponse>(`/api/manga/reading-events?source=${encodeURIComponent(sourceName)}&series_id=${encodeURIComponent(seriesId)}&chapter_id=${encodeURIComponent(chapterId)}`, { method: "POST", body: JSON.stringify({ chapter }) }),
